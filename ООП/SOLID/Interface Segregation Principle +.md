#oop
Относится к тем случаям, когда классы имеют "жирный интерфейс", то есть слишком раздутый интерфейс, не все методы и свойства которого используются и могут быть востребованы. Клиенты не должны зависеть от методов, которыми не пользуются.
...
## Пустые методы
Одним из типичных нарушений данного принципа являются нереализованные методы. Подобные методы обычно говорят о том, что в проектировании системы есть недостатки и упущения. Кроме того, нереализованные методы нередко также нарушают [[Liskov Substitution Principle +]].
...
## Другое
Принцип разделения интерфейсов решает проблему типов, которые могут делать разные вещи. Некоторые классы, хотя и представляют собой единую сущность, могут делать вещи, которые концептуально можно сгруппировать в разные интерфейсы.
